...
train_dataset_file_path= os.path.expanduser("/share/catalyst/ocp_datasets/data/is2re/10k/train/data_mod1.lmdbz")
...
val_dataset_file_path = os.path.expanduser("/share/catalyst/ocp_datasets/data/is2re/all/val_ood_both/data_mod1.lmdbz")
...
for i in range(epochs):
    loss.append(train(model, training_generator, optimizer, criterion, epoch=i, writer=writer, device=device))
    loss_eval.append(evaluate(model, valid_generator, criterion, epoch=i, writer=writer, device=device))
    path = 'SpinConv_identical_for_var_vol_tag_v1/' + '_'.join((timestamp, 'epoch', str(i), '.pickle'))
